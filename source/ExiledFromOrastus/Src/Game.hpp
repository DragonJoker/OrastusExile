...
namespace orastus
{
	namespace main
	{
		...
		public:
			...
			bool BuildTower( Castor::Point3r const & p_position, AbilitySet && p_abilities );
			void Spend( uint32_t p_value );
			void Gain( uint32_t p_value );
			void LoseLife( uint32_t p_value );

			TowerRPtr SelectTower( Cell const & p_cell );
			...
			inline bool HasSelectedTower()const
			{
				return m_selectedTower != nullptr;
			}

			inline Tower & GetSelectedTower()const
			{
		...
		private:
			...
			Castor3D::MeshSPtr DoSelectMesh( AbilitySet & p_category );
			void DoAddTower( Cell & p_cell, AbilitySet && p_category );
			void DoUpdateTowers();
